	...
	public void update(double elapsedTime, Dimension bounds,
		...
		if (getStatus()) {
			...
			for (Mass m : allMasses) {
				double angle = angleBetween(center.x - m.getX(),
						center.y - m.getY());
				double distance = Math.sqrt((center.x - m.getX())
						* (center.x - m.getX()) + (center.y - m.getY())
						* (center.y - m.getY()));
				Vector force = new Vector(angle, myMagnitude / distance
						* myExponent);
				m.applyForce(force);
			}
		}

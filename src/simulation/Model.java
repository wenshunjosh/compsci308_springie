...
public class Model {
	...
	private Vector[] myForces = { new GravityForce(), new ViscosityForce(),
	...
	public void update(double elapsedTime) {
		Dimension bounds = myView.getGameSize(); // update in order to change
													// size
		for (Spring s : mySprings) {
		...
		myTimer2 = System.currentTimeMillis(); // "Cools down" the keyboard so
												// one press will only result in
												// one update.
		int keyEvent = 0;
		if (myTimer2 - myTimer1 >= KEY_COOL_DOWN_TIME) {
			myTimer1 = myTimer2;
			if (!modelLoaded) {
				keyEvent = myView.getLastKeyPressed();
			}
			...
			if (myWallSizeKeys.contains(keyEvent)) { // duplicate with the above
														// if statement??

...
/**
 * This class represents text that appears on the screen and adds some utility
 * functions to the Image class.
 * 
...
public class Text {
	 ...
	 * and color, displaying the given text string.
	 */
	...
	public void paint(Graphics2D pen, Point2D center, Color color) {
		...
		bounds.setRect(-bounds.getWidth() / 2, -height / 2, bounds.getWidth(),
				bounds.getHeight());
		// draw text
		layout.draw((Graphics2D) pen, (float) (center.getX() - bounds
				.getWidth() / 2), (float) (center.getY() + bounds.getHeight()
				/ 2 - layout.getDescent()));
		// unset attributes
